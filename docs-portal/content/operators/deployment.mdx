...
Password reset and email verification tokens are stored in OSS tables. Deployments upgrading from the older Enterprise auth email implementation do not migrate active `ee_password_reset_tokens` or `ee_email_verification_tokens` rows. In practice, reset or verification links sent before this deployment stop working after the migration runs. Users should request a new password reset or verification email after the rollout.
...
In practice, a blocked migration metadata check should now fail with an application log that names startup migrations as the failing phase. It should not look only like a platform port-listen failure. A lock timeout usually means another database session is holding or waiting on the migration metadata table. The migration SQL body disables those local metadata timeouts so large index builds and backfills can still finish.

The current backend still owns migrations during startup. A separate pre-deploy migration job is a future deployment architecture improvement, not a requirement for the current production contract.
...
6. Ask users with old Enterprise reset or verification links to request fresh links.
