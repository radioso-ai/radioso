...
You write each step in plain language. Type `@` to drop in a value the routine collects (`@email`, `@order_id`). A skill call, an approval, a condition, a hand-off, or an ending is its own step or branch, added from the step menu. You never type curly braces or arrows.
...
Validation reports problems in author terms: an unreachable step, an undeclared variable, a tool step naming a skill the agent does not have. The operator test chat runs the agent's draft, so you try the change on a real message before anyone else meets it. **Review & Publish** on the agent is what makes it live — the routine ships in one reviewed snapshot with the directives, skills, and context variables you changed alongside it. Conversations already running keep the revision they started on, so nobody mid-flow gets swapped onto new steps.
