...
The job state moves from `queued` to `applying`, then to `applied`, `failed`, or `compensated`. `compensated` means the worker removed an agent left behind by an interrupted import; the response keeps the original agent id so support can correlate the cleanup. Set `AGENT_BUNDLE_IMPORT_ORPHAN_AGE_MS` to control how old an applying job must be before that worker sweep runs. The default is 15 minutes.
