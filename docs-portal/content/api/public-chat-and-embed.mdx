...
Use `GET /api/v1/public/chat/{token}/tail/{conversationId}` to poll for messages created after a cursor. Omit `cursor` once to establish a baseline; the response contains no messages and returns the current newest cursor. Then send that cursor on later polls to receive new visitor, AI, or human-agent messages. Public tail responses do not include operator ownership metadata.
