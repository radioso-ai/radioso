...
Deploy the frontend release carrying the callback route before you apply the credentials. Terraform reads `TF_VAR_frontend_image` from the frontend service already running and never advances it, so credentials applied first flip `/api/v1/ee/auth/google/status` to enabled and show the button while the browser still lands on a 404 on the way back from Google.
