...
`chatSessionId` and the older `anonymousSessionId` are accepted as legacy client hints but are not trusted as resume credentials. New clients normally should not send either; use `resumeToken` to resume and the returned `publicSessionToken` for chat requests.
