...
describe("test execution OpenAPI contract", () => {
  it("documents private fenced start, Eval capture, retain-one-side, message, and failed-side retry operations", () => {
    ...
    const capture = document.paths?.["/api/v1/agents/{agentId}/test-executions/{executionId}/sides/{sideId}/eval-snapshots/{messageId}"]?.post;
    ...
    expect(capture?.operationId).toBe("captureAgentTestExecutionEvalSnapshot");
    expect(capture?.responses?.["201"]?.content?.["application/json"]?.schema).toBeDefined();
