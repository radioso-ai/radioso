...
| Quality review queue | `quality.feedback_changed`, `quality.triage_changed` | P1 | `quality-view.tsx`. The same two events already feed the needs-attention **count**; the queue itself is the surface an operator sits on while triaging, and it is load-once on its filters. Both the rollup and the row page refetch in the background — no spinner, no error banner, previous data retained on failure — and the refetch is held while a close-review popover is open or a triage write is in flight, because swapping rows would strand the popover's anchor. A turn can also enter the queue on its signal alone with no quality write behind it; subscribing to `conversation.updated` would refetch two aggregates per assistant turn, so that slice rides a 60s reconcile floor instead. |
| Knowledge source list | `crawl.status_changed` | P1 | `document-sources-view.tsx`. The existing 5s crawl poll is gated on a crawl the view already knew about at mount, so a crawl started by a schedule, a connector, or another operator never woke it. The push covers start/stop/pause; `crawl.progress` stays off this surface and on the cheaper 5s poll, since it fires per checkpoint. Document counts and last-synced timestamps also drift from ordinary uploads and deletions, which are far too frequent to refetch the list per document — a 60s floor carries them. |
...
All candidate surfaces raised in review are resolved in the table: none of usage/trends,
