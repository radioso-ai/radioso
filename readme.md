...
Operators can review assistant-answer quality with `GET /api/v1/quality/turns`. Use `sort=negative_feedback_updated_at` with `activeNegativeFeedbackOnly=true` for an active thumbs-down queue: results are ordered by the latest feedback creation or edit, and feedback newer than an earlier resolved or dismissed decision is treated as open. Each turn includes `feedback.latestDownUpdatedAt`, while written feedback entries include both `createdAt` and `updatedAt`. Admins and owners can update an answer's triage state with `PUT /api/v1/quality/turns/<assistant-message-id>/triage`, using one of `open`, `acknowledged`, `resolved`, or `dismissed`.
