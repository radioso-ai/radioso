...
export function AgentAreaSubNav({ accountId, routeState }: { accountId: string; routeState: DashboardRouteState }) {
  ...
  useEffect(() => {
    ...
    let active = true
    void loadAgentCreationActionDefinitions().then((definitions) => { if (active) setCreationDefinitions(definitions) }).catch(() => undefined)
    return () => { active = false }
  }, [])
  ...
  return (
    <div className="ml-2 space-y-1 border-l border-sidebar-border/70 pl-2">
      ...
      <AgentBundleImportDialog open={importOpen} onOpenChange={setImportOpen} onImported={() => void agentsApi.listAgents().then((result) => setAgents(result.agents)).catch(() => undefined)} agentSettingsHrefBuilder={(agentId) => agentHref(accountId, routeState, agentId, 'profile', workspaceParts.workspaceId, workspaceParts.workspacePublicRouteKey)} />
