...
Instead of an answer, the assistant returns one short waiting line, such as
"a teammate is joining, please wait." The line is generated by the model in the
conversation's language, so it is multilingual, and it repeats on each further
visitor message. If the line cannot be generated, the visitor turn produces no
reply. The AI stays suppressed until an operator explicitly hands the
conversation back.
