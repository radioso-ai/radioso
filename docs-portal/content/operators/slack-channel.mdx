...
The Slack channel lets people talk to your Radioso agents from Slack. One Slack workspace maps to one Radioso organization. Inside it, the default agent answers direct messages and any channel without a specific binding, and individual Slack channels can be routed to specific agents.
...
- When a turn's outcome is `no_context` and an escalation channel is configured, Radioso posts a human follow-up to that channel.
