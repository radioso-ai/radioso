...
- **Anti-Goals**: Do not merge conversation mode into `answerSupportPolicy`. Do not add generic non-grounded fallback behavior. Do not push exploration logic into route handlers or frontend components. Do not implement expansive behavior as an uncontrolled single-prompt blob with no bounded output rules or observability.
...
- **FR-008**: The active `answerSupportPolicy` MUST remain a separate control and MUST continue to govern how unsupported substantive content is handled after answer generation.
