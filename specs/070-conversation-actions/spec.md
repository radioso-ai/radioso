...
5. **Per-agent toggle, opt-in** (host + UI) — _done_: `contactRoutineModule` (a built-in in the default composition) wires the feature, gated by a per-agent `contactRequestsEnabled` flag (agent behavior JSONB, **default off**). An **intake advertiser** surfaces the existing public-chat "contact a human" button — but only for agents that enabled the capability — and never claims the turn (`handle` → null), so the turn falls through to the engine where the routine activates on the `intent_click`. The routine activator gates on the same flag (defence in depth). The recipient is the **workspace owner** (`WorkspaceOwnerContactRecipientResolver`, falling back to an admin) — no extra config.
   - **UI**: the Skills tab is no longer Enterprise-gated. OSS shows a generic "Contact requests" toggle bound to the per-agent flag; EE keeps its richer "Contact handoff" config. The public-chat button follows the advertised action (no edition gate), so it works in OSS when enabled; EE behaviour is unchanged. The OpenAPI spec + TypeScript SDK + MCP types were regenerated for the new field.
   - **EE coexistence**: EE's human-contact intake runs pre-engine and claims the turn first, so the OSS routine stays dormant in EE; the advertised action de-duplicates.
   - **Resolved decisions**: opt-in per agent (default off); recipient = workspace owner email (swappable by registering a custom handler); trigger = the existing public-chat contact button via a Skills-tab toggle. **Known follow-up**: a routine-claimed turn still runs retrieval during session prep (a later optimization).
