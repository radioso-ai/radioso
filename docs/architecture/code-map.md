...
- `backend/src/modules/operatorCopilot/tools/triage.ts` and `triageDigest.ts` (`workspace_triage` and `needs_attention`: per-source reads and their permissions, the ranking the digest feeds, and the working list's action handles)
