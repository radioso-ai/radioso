...
export function RoutineStepRow({ step, stepIndex, slotNames, editable = false, editing, onEditInstruction, onEditBinding, onEditApproval, onEditBranch, onEditStep, instructionEditor, bindingEditor, approvalEditor, branchEditor, stepEditor }: {
  ...
  return <li className="rounded-lg border border-border p-4"><div className="flex items-center gap-2 text-sm font-medium text-foreground"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-muted text-xs">{stepIndex + 1}</span>{step.kind === 'approval' ? <ListChecks className="h-4 w-4" /> : step.kind === 'tool' || step.kind === 'action' ? <Wrench className="h-4 w-4" /> : <Bot className="h-4 w-4" />}<button type="button" aria-label={label} onClick={onEditStep} disabled={!editable} className="group text-left disabled:cursor-default"><span>{label}</span><EditHint editable={editable} /></button></div>{editing === 'step' ? stepEditor : <div className="mt-3">{editing === 'instruction' ? instructionEditor : <button type="button" aria-label="Instruction" onClick={onEditInstruction} disabled={!editable} className="group block w-full text-left disabled:cursor-default"><InstructionSentence segments={step.instruction} editable={editable} /><EditHint editable={editable} /></button>}</div>}{step.kind === 'tool' || step.kind === 'action' ? <div className="mt-3">{editing === 'binding' ? bindingEditor : <button type="button" aria-label="Bindings" onClick={onEditBinding} disabled={!editable} className="group flex items-center gap-1 text-left text-xs text-muted-foreground disabled:cursor-default"><ArrowRight className="h-3.5 w-3.5" />{formatBindingLine(step.inputBindings, step.outputAssignments) ?? 'uses nothing → sets nothing'}<EditHint editable={editable} /></button>}</div> : null}{step.kind === 'approval' ? <div className="mt-3">{editing === 'approval' ? approvalEditor : <button type="button" aria-label="Approval choices" onClick={onEditApproval} disabled={!editable} className="group block w-full rounded-md bg-violet-50 p-3 text-left text-sm text-violet-950 disabled:cursor-default dark:bg-violet-500/10 dark:text-violet-100"><p className="font-medium">A person chooses:<EditHint editable={editable} /></p><ul className="mt-2 space-y-1">{(step.options ?? []).map((option) => <li key={option.id}>{option.label}{option.description ? ` — ${option.description}` : ''}</li>)}</ul></button>}</div> : null}{step.branches.length > 0 ? <ul className="mt-4 space-y-2 border-l border-border pl-3">{step.branches.map((branch, index) => <RoutineBranchRow key={`${step.stableStepId}-${index}`} branch={branch} slotNames={slotNames} editable={editable} editing={editing === `branch:${index}`} onEdit={() => onEditBranch?.(index)} editor={branchEditor?.(index, branch)} />)}</ul> : null}</li>
}
