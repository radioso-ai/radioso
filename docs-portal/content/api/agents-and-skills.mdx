...
Agents are the user-facing conversational agents in a workspace. Skills describe the product work Radioso can perform through stable contracts such as assistant chat, retrieval answer, document search, and MCP tools.
...
Retrieval answer configuration lives on the agent. Use `skillSettings["retrieval.answer"]` to set fields such as query rewrite behavior, reranking, metadata rules, strategy, and answer instruction for that one agent. An empty `skillSettings` map means the agent inherits system/model defaults.
