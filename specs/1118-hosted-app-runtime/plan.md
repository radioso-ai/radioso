...
**Configuration fields.** `type` is one of `text | number | boolean | select | url | connection_slot`; `connection_slot` fields reference a declared slot. Secrets never appear here.
...
**Destinations.** `host` is either `{ kind: "pattern", pattern }` or `{ kind: "configuration", field }` (installation-bound host taken from a `url` configuration field). Plus `protocols` (`https` only in Release A), `ports?`, `purpose`, `dataClasses[]`, `connectionSlot?`.

**Storage collections.** `scope: "installation"`, `schemaVersion` (integer), `compatibleReaderVersions[]`, `recordSchema { fields[{ key, type: string|number|boolean|timestamp|json, required }] }`, `indexes[{ id, field }]` (scalar fields only), `quotas { maxRecords, maxRecordBytes }`, `retention { kind: none|ttl, seconds? }`, `allowedOperations` subset of `get | put | delete | query_by_index`.

**Contributions.** Common header: `id`, `kind`, `displayName`, `description`, `permissions[]` (subset of manifest permissions), `egressDestinations[]`, `deadlineMs`, `availability: optional | required`. Kinds fully specified in Release A:
- `document_source`: `externalIdNamespace`, `syncModes[]` of `push | poll | backfill`, `contentFormats[]` of `html | markdown | text`, `indexedFieldKeys[]`, `backfill?: { checkpointCollection }`. Execution class `scheduled_task` for backfill runs.
- `external_webhook_handler`: `authentication { kind: "hmac_sha256", secretConnectionSlot, signatureHeader, signaturePrefix? }`, `maxBodyBytes`, `replayWindowSeconds`. Execution class `external_webhook`.
- `scheduled_task`: `schedule` is `{ kind: "interval", seconds }` or `{ kind: "interval_from_configuration", field, minSeconds }`, `overlapPolicy: skip | queue | replace`, `maxDurationSeconds`, `retry { maxAttempts, backoff { kind: "exponential", baseSeconds, maxSeconds } }`, `checkpointCollection?`. Execution class `scheduled_task`.
...
**Runtime protocol.** `InvocationRequest { protocolVersion, invocationId, installationId, contributionId, executionClass, attempt, idempotencyKey, deadlineAt, identity { token, expiresAt }, input }` where `input` is discriminated by contribution kind: webhook `{ deliveryId, receivedAt, headers, body }`, scheduled `{ occurrenceId, scheduledFor, checkpoint? }`, backfill `{ requestId, checkpoint? }`. `InvocationResponse { protocolVersion, invocationId, outcome: completed | failed | retry, output? { checkpoint?, counts? { ingested, deleted, skipped } }, error? { code, message }, retryAfterSeconds? }`. Host capability requests (App to gateway) are a discriminated union: `documents.ingest`, `documents.delete`, `storage.get`, `storage.put`, `storage.delete`, `storage.query`, `egress.fetch`; responses are `{ ok: true, result } | { ok: false, error { code, message } }` with error codes `denied | not_found | invalid_input | quota_exceeded | version_conflict | destination_denied | deadline_exceeded | unavailable | internal`. Messages are bounded and must not echo payloads.
...
**validateManifest(manifest, policy)** returns `{ ok: true, manifest } | { ok: false, issues[{ code, path, message }] }`. Policy carries `supportedContributionKinds`, `supportedConnectionKinds`, `supportedPermissions`. Cross-reference checks: unique ids per family; contribution permissions ⊆ manifest permissions ⊆ policy permissions; `egressDestinations`, `secretConnectionSlot`, `checkpointCollection`, `connection_slot` fields, and `interval_from_configuration.field` all resolve; configuration-bound destination hosts reference `url` fields; interval fields are `number`; index fields exist and are scalar; a webhook handler requires `egress` nothing but a `generated_secret` or `secret_fields` slot; a `document_source` with `backfill` names an existing collection.
