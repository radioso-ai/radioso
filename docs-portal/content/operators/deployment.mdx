...
The included staging Terraform also enables PostHog error reporting with `ERROR_SINKS=audit,posthog`. Set `POSTHOG_API_KEY` for staging. `POSTHOG_HOST` defaults to the US ingestion host and can be overridden for another PostHog region or a self-hosted instance.
