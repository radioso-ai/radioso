...
1. **Given** a conversation awaiting a human, **When** the operator opens the Inbox, **Then** they see a two-pane page with the item in the left queue and, on selection, a response view on the right.
...
- **FR-004**: The Inbox MUST render as a two-pane page with a lens toggle at the top of the left pane: the Needs you lens (default) shows the open-item queue (critical items first, oldest-waiting on top) with a collapsed recently-closed strip; the All lens lists every conversation. A response view renders on the right.
...
- **FR-013**: The top-level Inbox navigation entry MUST show the count of open inbox items.
