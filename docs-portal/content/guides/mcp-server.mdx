...
The tool's text content is the agent's answer. Its `structuredContent` is the **agent reply envelope**, so a calling model can act on the outcome instead of guessing from prose:
...
- `routine` is present when the turn ran a routine. `status` is `active`, `waiting_for_input`, `waiting_for_approval`, `completed`, or `abandoned`; `pendingInput` lists every required slot still open plus the current step's optional ones, each with `key`, `type`, `required`, and `description`, so the client can supply all of them in one message.
- `traceId` is the turn's trace id, the one an operator sees in Activity.

The REST agent channel, `POST /api/v1/agents/{agentId}/chat`, returns the same `answerCoverage`, `ownership`, `routine`, and `traceId` fields beside its `answer` string and `citations` array. Both routes publish the envelope as `AgentReplyEnvelopeCore` in the OpenAPI document.
...
The routine starts with those slots filled, skips the steps that would have asked for them, and reports where it landed in the envelope's `routine` field (`toolName` names the tool). Input is validated against the descriptor before anything is recorded: an unknown tool returns `404` with `error.details.code` `routine_tool_unknown`; mismatched input returns `400` with `routine_invocation_invalid` and one `{ path, code }` entry per field (`required`, `type`, `format`, `unknown_field`). A conversation keeps the release it started on, so a routine exposed after that shows up in the catalog but runs from the next session. Calling a tool whose routine already completed in the conversation under **Once per conversation** answers normally and still reports `routine.status: "completed"`, so the client learns why nothing started.

An MCP client gets the same catalog as tools. The standalone server reads it once, when it exchanges the credential for a session, and the session's `tools/list` is `ask_agent`, the two documentation tools, and one tool per exposed routine — `start_return(orderId, reason?)` in the example above — each carrying the operator's description and the descriptor's JSON Schema as its input schema. Calling a routine tool checks the arguments against that schema first (a call missing a required slot comes back as a tool error before the backend is involved), then runs the invocation above. The result's `structuredContent` is the full agent reply envelope and its text content is `answer.text`, exactly as for `ask_agent`, so `routine.status` and `pendingInput` read the same way whichever tool the client called.

The catalog is fixed for the session's lifetime: a routine exposed, renamed, or withdrawn afterwards appears when the client's next session is established, and the server sends no `notifications/tools/list_changed`.
...
- **An exposed routine is missing from `tools/list`.** The session opened before the routine was exposed and published. The catalog is read once per session, so let the session expire or exchange the credential again and the tool is listed.
