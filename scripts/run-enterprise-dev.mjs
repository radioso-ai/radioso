...
const defaultEnterpriseRoot = path.resolve(repoRoot, "../radioso-enterprise");

const usage = `Usage: ./run-enterprise-dev.sh

Starts the local enterprise development stack:
...
Environment:
  ...
  RADIOSO_ENTERPRISE_APP_ORIGIN Public frontend origin. Defaults to http://localhost:3000.
`;
...
const updateEnvFile = async (filePath, updates) => {
  ...
  if (pending.size > 0) {
    ...
    nextLines.push("# Enterprise local development");
...
const main = async () => {
  ...
  const enterpriseRoot = path.resolve(process.env.RADIOSO_ENTERPRISE_REPO ?? defaultEnterpriseRoot);
  const backendDir = path.join(repoRoot, "backend");
  const frontendDir = path.join(repoRoot, "frontend");
  const envPath = path.join(backendDir, ".env");
  const enterpriseBackendPackage = path.join(enterpriseRoot, "packages/backend-module");
  const enterpriseWidgetPackage = path.join(enterpriseRoot, "packages/embed-widget");
  const appOrigin = process.env.RADIOSO_ENTERPRISE_APP_ORIGIN ?? "http://localhost:3000";

  for (const requiredPath of [enterpriseRoot, enterpriseBackendPackage, enterpriseWidgetPackage]) {
    if (!(await pathExists(requiredPath))) {
      throw new Error(`Missing enterprise path: ${requiredPath}`);
    }
  }
  ...
  await command("npm", ["install", "--no-audit", "--no-fund"], { cwd: enterpriseRoot });
  await command("npm", ["run", "build"], { cwd: enterpriseRoot });
...
try {
  ...
  const message = error instanceof Error ? error.message : "Unexpected enterprise startup failure";
