...
`GET /api/v1/agents/{agentId}/skill-capabilities` returns the capability registry projection for the agent. Each capability includes its targets, input schema, outcomes, supported invocation modes, and availability.
...
Create skills with `POST /api/v1/agents/{agentId}/skills`. The form and API use the same model for `retrieve`, `mcp_tool`, `email`, `slack_post`, `webhook_call`, and `notify`.
