...
`OPERATOR_MCP_RESOURCE_URL` must be the exact `/operator/mcp` URL with no query or fragment. `OPERATOR_MCP_ISSUER_URL` is the public Radioso application origin that serves authorization and consent. The frontend value is read at request time and should match the resource the standalone MCP process serves. The GitHub Terraform workflow discovers the Cloud Run MCP URL unless `OPERATOR_MCP_PUBLIC_ORIGIN` supplies a custom domain.
...
The dashboard exposes `operator:read`, `operator:probe`, and `operator:propose` through the current limited catalog. Leave `OPERATOR_MCP_ROLLOUT_WORKSPACE_IDS` unset or empty to make an enabled Operator MCP deployment available in every workspace. Set one or more workspace UUIDs only for a staged rollout. `OPERATOR_MCP_VERIFICATION_BUDGET_PER_MINUTE` accepts 1 through 6 and defaults to 6. The API access card reports disabled, misconfigured, or unavailable states without offering a personal-token workaround.
