...
- `bootstrapGreeting` for bootstrap flows
...
Use `GET /api/v1/chat/history` to list saved conversations.

Use `GET /api/v1/chat/history/{conversationId}` to retrieve one conversation and its debug metadata.
