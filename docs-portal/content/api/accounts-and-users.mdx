...
description: List accounts, invite users, manage organization roles, switch account context, and review support access.
---
...
3. List current users, invitations, workspace grants, and recent support access for that account.
...
`GET /api/v1/account/users` also returns recent Radioso support impersonation sessions for the active account.

Owners can use this data to see whether support access is active, why it was approved, and when it expires.

Support staff access is controlled by the `SUPPORT_STAFF_EMAILS` environment variable. A support session is not an account membership and expires after 24 hours.
