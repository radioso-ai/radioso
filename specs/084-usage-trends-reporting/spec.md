...
- **Performance Rule**: Queries MUST use existing indexes where possible (`idx_conversations_workspace_agent_updated_id`, `messages_workspace_role_created_id_idx`, `idx_usage_events_workspace_occurred_at`, `idx_usage_events_account_occurred_at`). The API MUST bound the requested range / bucket count. New indexes MAY be added if a query plan requires them.
