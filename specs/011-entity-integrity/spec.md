...
- **Encapsulation Rule**: [chatService.ts](/Users/dm/code/radioso-entity-integrity/backend/src/modules/chat/services/chatService.ts) must remain orchestration-only and must not absorb entity parsing, ranking, or verification rules. [promptBuilder.ts](/Users/dm/code/radioso-entity-integrity/backend/src/modules/retrieval/services/promptBuilder.ts) must remain prompt assembly only and must not become the primary home for retrieval policy.
