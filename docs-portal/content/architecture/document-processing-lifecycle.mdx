...
Metadata extraction is optional and disabled by default. When enabled by the workspace setting, a source override, or a reprocess override, the worker makes one model call for the document. The call understands the document type and extracts supported structured tags such as event dates.

The extraction result is validated before it changes metadata. Event dates are attached to overlapping chunks as `dateFrom` and `dateTo`. Article publication dates can be attached at document level. If validation or the provider call fails, the document continues through processing without extracted tags.
