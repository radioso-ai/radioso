...
## Three views of the same draft

The editor has three views of one draft. Switching between them re-projects the same routine; it does not make a copy.

- **Prose** is the primary view. You write the routine in plain language and drop inline chips where structure is needed.
- **Document** lays the routine out as a readable flow with typed controls for the parts that carry structure.
...
Document starts with a **Starts when** line, followed by **Information**, numbered steps, and endings. A skill step shows its **uses → sets** bindings. Each branch is marked **Rule** for an exact check or **AI decides** when the agent judges the condition, then points to another step, a finish, or a hand-off message.

Edit the starts-when line directly. In a step instruction, type `@` to capture a value and choose its type in **Information**. Use **+ Step** to add chat, skill, approval, or action steps. Condition rows set the branch decision and target; ending rows set the message the flow delivers.
