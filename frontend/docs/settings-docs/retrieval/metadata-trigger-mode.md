...
Trigger matching happens during query interpretation, and if the agent has no trigger-based rules at all, Radioso skips that step rather than paying for it. The match is decided by a model completion, and that decision is authoritative.
