...
test("opens Ray, streams activity and an answer, resumes history, and deletes it", async ({ page }) => {
  ...
  await page.route("**/backend/api/v1/copilot/**", async (route) => {
    ...
    if (path === "/copilot/availability" && request.method() === "GET") {
      await route.fulfill({ json: { available: true, reason: "ok", canManage: true, applyableProposalTargets: ["directive","agent_setting","routine","agent_skill","context_variable","document","ingestion_settings","website_crawl","agent_greeting"] } });
      return;
    }
...
test("asks Ray about selected dashboard text", async ({ page }) => {
  ...
  await page.route("**/backend/api/v1/copilot/**", async (route) => {
    const request = route.request();
    const path = new URL(request.url()).pathname.replace("/backend/api/v1", "");
    if (path === "/copilot/availability") return route.fulfill({ json: { available: true, reason: "ok", canManage: true, applyableProposalTargets: ["directive","agent_setting","routine","agent_skill","context_variable","document","ingestion_settings","website_crawl","agent_greeting"] } });
...
test("reviews a directive proposal, expands its diff, applies it, and opens the target", async ({ page }) => {
  ...
  await page.route("**/backend/api/v1/copilot/**", async (route) => {
    const request = route.request();
    const path = new URL(request.url()).pathname.replace("/backend/api/v1", "");
    if (path === "/copilot/availability" && request.method() === "GET") return route.fulfill({ json: { available: true, reason: "ok", canManage: true, applyableProposalTargets: ["directive","agent_setting","routine","agent_skill","context_variable","document","ingestion_settings","website_crawl","agent_greeting"] } });
