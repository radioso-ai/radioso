...
const dependencies = () => {
  const previewChange = vi.fn(async (input: { kind: string }) => ({ before: input.kind === "create" ? null : directive, after: input.kind === "remove" ? null : { ...directive, enabled: input.kind === "set_enabled" ? false : true }, coherence: input.kind === "remove" ? null : { coherent: true, conflicts: [], rationale: "Coherent." }, referencedBy: [] }));
  return {
    ...
    directiveAuthor: { draftForProposal: vi.fn(async () => ({ draft: { directive: { name: "quote-primary", condition: { kind: "always" as const }, action: "Use **this exact text**.", excludes: [], tags: [], surfaces: [] } }, versionToken: "2026-09-26T09:00:00.000Z", current: null })), readProposalFence: vi.fn(async () => "2026-09-26T09:00:00.000Z") }, directives: { previewChange },
  };
};
