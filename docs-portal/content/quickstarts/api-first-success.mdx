...
Use the session-authenticated API-access route to create a role-bounded personal token with an expiry no more than 90 days away. Its secret is shown once; inventory responses cannot recover it later. For unattended workspace automation, create a service account under **Settings → Service accounts** instead.
