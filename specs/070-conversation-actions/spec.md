...
5. **Per-agent toggle, opt-in** (host + UI) — _done_: `contactRoutineModule` (a built-in in the default composition) wires the feature, gated by a per-agent `contactRequestsEnabled` flag (agent behavior JSONB, **default off**). An **intake advertiser** surfaces the existing public-chat "contact a human" button — but only for agents that enabled the capability — and never claims the turn (`handle` → null), so the turn falls through to the engine where the routine activates on the `intent_click`. The routine activator gates on the same flag (defence in depth). The recipient is the **workspace owner** (`WorkspaceOwnerContactRecipientResolver`, falling back to an admin) — no extra config.
   - **UI**: the Skills tab is no longer Enterprise-gated. OSS and EE show the same generic "Contact requests" toggle bound to the per-agent flag. The public-chat button follows the advertised action (no edition gate), so it works when enabled. The OpenAPI spec + TypeScript SDK + MCP types were regenerated for the new field.
