...
2. **Given** the same agent, **When** `GET /.well-known/mcp-server-card/{publicId}.json` is fetched, **Then** the response follows the MCP server-card draft (SEP-2127) and points at the same endpoint.
...
3. **Given** walk-in traffic, **When** one source exceeds the per-source or per-agent budget, **Then** further exchanges and turns are throttled through `AbuseControlService`, and the throttle is visible in the security event feed.
...
- Catalog changes while a session is open → the session keeps its catalog; the next session sees the new one. `notifications/tools/list_changed` is a follow-up.
...
- **FR-011** The validator MUST refuse: duplicate `toolName` within an agent; reserved names; `toolName` changed after the lineage's first published version with exposure enabled (`exposure_tool_name_changed`); exposure on a routine with an activation gate (`exposure_requires_ungated_activation`).
...
- **FR-020** `GET /.well-known/agent-card/{publicId}.json` MUST serve an A2A-conformant Agent Card for a published agent: `name`, `description`, `url` (MCP endpoint), `securitySchemes` (`none` iff walk-in enabled; `bearer` always listed), `skills[]` from descriptors (`id = toolName`, `name`, `description`), `documentationUrl` (connect guide).
- **FR-021** `GET /.well-known/mcp-server-card/{publicId}.json` MUST serve the MCP server-card draft shape for the same agent and endpoint.
...
- **FR-031** When enabled, the MCP session exchange MUST accept the public id with no secret and issue a session bound to a fresh conversation with `sourceChannel = "mcp"`, `callerKind = "agent"`; when disabled the exchange MUST be refused without revealing whether the agent exists beyond what the card already says.
...
- **FR-031a** Walk-in sessions MUST expire after an idle period (default 24 h) and MUST be invalidated when `publicAgentAccessEnabled` is turned off or the public id is rotated: every converse request re-checks the flag and the id, mirroring how credential-bound sessions re-check their credential.
...
- **FR-033** Converse `ask` MUST accept `signedIdentity`, verified with the MCP session id as the bound session, producing the same `verifiedCustomerId` / `verifiedIdentity` as the embed path.
- **FR-034** Walk-in conversations MUST be metered like every other conversation through the conversation-metering path (#1246): same quota, same cost. There is no separate bucket for agent callers; usage rows carry `callerKind` so the split is observable. Protection against a runaway caller is the per-agent walk-in rate budget in FR-032, not the quota.

### Resumption

- **FR-040** `GET /api/v1/mcp/converse/messages?since=<messageId>&waitMs=<0..25000>` (session-bound) MUST return messages after `since` with `id`, `author` (`agent` | `human`), `createdAt`, `text`, and the current `ownership.state`; with `waitMs` it MUST long-poll on the conversation event bus.
...
- Server-card path: draft currently says `<endpoint>/server-card`; deployed cards use `/.well-known/mcp-server-card/...`. Decide at plan time from the then-current draft.
