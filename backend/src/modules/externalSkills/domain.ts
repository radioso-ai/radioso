 ...
 */
const serverUrlSchema = trimmedText(EXTERNAL_SKILLS_LIMITS.serverUrl).superRefine((value, ctx) => {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "serverUrl must be a valid URL" });
    return;
  }
  if (url.protocol !== "https:") {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "serverUrl must use https" });
  }
  if (url.username !== "" || url.password !== "") {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "serverUrl must not embed credentials (userinfo)" });
  }
});
