-- One-time data migration (071 US4): snapshot each tuned workspace retrieval_settings value
-- onto every agent's skill_settings["retrieval.answer"], only where it differs from the new
-- system default AND the agent has no explicit key for it. Wrapped in a guard so that
-- re-applying the migration set after migration 081 has dropped the source columns is a clean
-- no-op (the data migration has, by then, already run).
DO $mig$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_name = 'retrieval_settings'
      AND column_name = 'similarity_threshold'
  ) THEN
    RAISE NOTICE '080 skipped: workspace retrieval columns already removed (migration already applied)';
    RETURN;
  END IF;

  -- similarity_threshold cannot be expressed per-agent. If any tuned row has a non-default
  -- threshold, abort loudly with the offending workspace ids (production audit expects zero).
  IF EXISTS (
    SELECT 1
    FROM retrieval_settings rs
    WHERE rs.similarity_threshold IS DISTINCT FROM 0.2
      AND EXISTS (SELECT 1 FROM agents a WHERE a.workspace_id = rs.workspace_id)
  ) THEN
    RAISE EXCEPTION
