...
```bash
curl -sS -X POST http://localhost:8080/api/v1/document/import \
  -H "Authorization: Bearer $RADIOSO_API_TOKEN" \
  -F "file=@./handbook.pdf" \
...
Document detail responses include enrichment provenance when a processing run attempted or skipped AI document enrichment. The `enrichment` object reports safe operator fields such as status, detected shape, model, anchor date, fact count, applied chunk count, and failure reason. It does not include source text, prompts, completions, or chunk content.

Single-document reprocess accepts an optional enrichment override for that run:

```bash
curl -sS -X POST http://localhost:8080/api/v1/document/document-id/reprocess \
  -H "Authorization: Bearer $RADIOSO_API_TOKEN" \
  -H "Content-Type: application/json" \
...
## Source enrichment and reprocessing
...
```bash
curl -sS -X PATCH http://localhost:8080/api/v1/document/sources/source-id \
  -H "Authorization: Bearer $RADIOSO_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"documentEnrichmentOverride":"on"}'
```
...
```bash
curl -sS -X POST http://localhost:8080/api/v1/document/sources/source-id/reprocess \
  -H "Authorization: Bearer $RADIOSO_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"documentEnrichmentOverride":"on"}'
```
