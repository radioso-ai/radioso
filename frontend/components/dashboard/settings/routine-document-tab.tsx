...
import { instructionIsEmpty, RoutineDocumentHeader, RoutineEndingsSection, RoutineInformationSection, RoutineStepRow } from '@/components/dashboard/settings/routine-document-rows'
...
import { formatBindingLine } from '@/lib/routine-document'
import {
  addBranch,
  addStep,
...
function RoutineDocumentReader({ doc }: { doc: RoutineBlockDoc }) {
  ...
  return <article className="space-y-6 rounded-lg border border-border bg-background p-5" aria-label="Routine document">
    ...
    <section aria-labelledby="routine-document-steps"><h3 id="routine-document-steps" className="text-sm font-semibold text-foreground">Steps</h3><ol className="mt-3 space-y-3">{doc.steps.map((step, stepIndex) => <RoutineStepRow key={step.stableStepId} step={step} stepIndex={stepIndex} slotNames={slotNames} />)}</ol></section>
...
function GuardEditor({ branch, slots, onChange }: { branch: RoutineBlockBranch; slots: RoutineBlockDoc['information']; onChange: (patch: Partial<RoutineBlockGuard>) => void }) {
  ...
  if (guard.kind === 'field') {
    ...
    return <div className="grid gap-2 sm:grid-cols-4"><select aria-label="Rule variable" value={guard.fieldRef ?? ''} onChange={(event) => onChange({ fieldRef: event.target.value })} className="h-9 rounded-md border border-input bg-transparent px-2 text-sm"><option value="">Variable…</option>{slots.map((slot) => <option key={slot.key} value={slot.key}>{slot.key}</option>)}</select><select aria-label="Rule operator" value={op} onChange={(event) => onChange({ fieldOp: event.target.value as RoutineFieldGuardOp })} className="h-9 rounded-md border border-input bg-transparent px-2 text-sm">{['equals', 'not_equals', 'in', 'is_present', 'is_absent', 'is_true', 'is_false', 'gt', 'gte', 'lt', 'lte', 'older_than', 'within'].map((value) => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</select>{fieldGuardOpNeedsValue(op) ? <Input aria-label="Rule value" value={op === 'in' ? (guard.fieldValues ?? []).join(', ') : String(guard.fieldValue ?? '')} onChange={(event) => onChange(op === 'in' ? { fieldValues: event.target.value.split(',').map((value) => value.trim()).filter(Boolean), fieldValue: null } : { fieldValue: event.target.value, fieldValues: null })} placeholder={op === 'in' ? 'Values, separated by commas' : 'Value'} /> : null}{fieldGuardOpNeedsUnit(op) ? <select aria-label="Rule unit" value={guard.fieldUnit ?? 'days'} onChange={(event) => onChange({ fieldUnit: event.target.value as RoutineFieldGuardUnit })} className="h-9 rounded-md border border-input bg-transparent px-2 text-sm">{ROUTINE_FIELD_GUARD_UNITS.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select> : null}</div>
  }
...
function BranchEditor({ branch, branchIndex, step, doc, targetOptions, apply, onDone }: { branch: RoutineBlockBranch; branchIndex: number; step: RoutineBlockStep; doc: RoutineBlockDoc; targetOptions: (stepId: string) => ReactNode; apply: (edit: (current: RoutineBlockDoc) => RoutineBlockDoc) => void; onDone: () => void }) {
  return <div className="space-y-2"><div className="grid gap-2 sm:grid-cols-[130px_150px_1fr_auto]"><select aria-label="Decision kind" value={branch.guard.kind === 'llm' ? 'llm' : 'rule'} onChange={(event) => apply((current) => changeBranchGuardKind(current, step.stableStepId, branchIndex, event.target.value === 'llm' ? 'llm' : 'field'))} className="h-9 rounded-md border border-input bg-transparent px-2 text-sm"><option value="rule">Rule</option><option value="llm">AI decides</option></select>{branch.guard.kind !== 'llm' ? <select aria-label="Rule kind" value={branch.guard.kind} onChange={(event) => apply((current) => changeBranchGuardKind(current, step.stableStepId, branchIndex, event.target.value as RoutineGuardKind))} className="h-9 rounded-md border border-input bg-transparent px-2 text-sm">{guardKinds.map((kind) => <option key={kind} value={kind}>{kind.replaceAll('_', ' ')}</option>)}</select> : <span />}<select aria-label="Branch target" value={branch.target.kind === 'step' ? `step:${branch.target.stableStepId}` : `ending:${branch.target.terminalId}`} onChange={(event) => { const [kind, id] = event.target.value.split(':'); apply((current) => kind === 'step' ? targetBranchAtStep(current, step.stableStepId, branchIndex, id!) : referenceEnding(current, step.stableStepId, branchIndex, id!)) }} className="h-9 rounded-md border border-input bg-transparent px-2 text-sm">{targetOptions(step.stableStepId)}</select><Button type="button" variant="ghost" size="sm" aria-label="Remove condition" onClick={() => { apply((current) => removeBranch(current, step.stableStepId, branchIndex)); onDone() }}><Trash2 className="h-4 w-4" /></Button></div><GuardEditor branch={branch} slots={doc.information} onChange={(patch) => apply((current) => updateBranchGuard(current, step.stableStepId, branchIndex, patch))} />{branch.target.kind === 'ending' ? <div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => apply((current) => createEndingForBranch(current, step.stableStepId, branchIndex, 'complete'))}>New finish</Button><Button type="button" size="sm" variant="outline" onClick={() => apply((current) => createEndingForBranch(current, step.stableStepId, branchIndex, 'handoff'))}>New hand-off</Button>{branchEnding(branch) ? <Input aria-label="Ending message" value={branchEnding(branch)?.instruction ?? ''} onChange={(event) => { const ending = branchEnding(branch); if (ending) apply((current) => updateEnding(current, ending.stableStepId, { instruction: event.target.value })) }} placeholder="Ending message" /> : null}</div> : null}<Button type="button" size="sm" onClick={onDone}>Done</Button></div>
}
