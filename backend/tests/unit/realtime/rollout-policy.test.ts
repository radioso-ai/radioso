...
const otherAccountId = "5d7293c8-d241-4f8f-a4db-3df5b88da44d";
...
describe("realtime rollout policy", () => {
  it("handles disabled, internal, allowlist, and default-on synchronously", () => {
    ...
    expect(createRealtimeRolloutPolicy({ mode: "internal", accountIds: [accountId] }).allows({ accountId })).toBe(true);
  ...
  it("never treats an empty internal rollout account set as operational", () => {
    expect(createRealtimeRolloutPolicy({ mode: "internal", accountIds: [] }).allows({ accountId })).toBe(false);
    expect(createRealtimeRolloutPolicy({ mode: "internal", accountIds: [accountId] }).allows({ accountId: otherAccountId })).toBe(false);
  });
});
