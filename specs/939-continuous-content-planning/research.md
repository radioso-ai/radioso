...
| Cost control | Durable `audience_pulse.refresh` rate scope, default 3/15 minutes per account/workspace; existing answer usage reservation only around provider work | The general 60/minute expensive-route limit is too high. No traffic uses neither provider nor reservation; a validated model completion decides commit so a later snapshot write failure cannot make provider work unmetered. |
