...
1. Send a chat request with a personal token or service-account credential issued from Workspace → API access.
...
This calls the hosted EU API; use `https://api-us.radioso.ai` or your own self-hosted origin instead, matching the host where the personal or service credential was issued — see [Personal tokens and service accounts](/api/workspaces-and-tokens).
