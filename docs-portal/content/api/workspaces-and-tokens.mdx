...
The list endpoints and **Settings → API access** show the credentials that can authenticate today, so the count on screen is the count that runs against those limits. A credential leaves the list once you revoke it or once its expiry passes, and a credential suspended by a disabled service account stays listed because re-enabling the account restores it. Audit events keep every credential's history either way.
