...
- **FR-015a**: Every document effect an App requests MUST name the document-source contribution it writes through. The App Gateway MUST derive the invoking contribution from the capability session, MUST reject an effect whose source is not one the invoking contribution declared (or, for a backfill, is not the invoking source itself), MUST enforce that source's external-identity namespace and indexed-field policy, and MUST inject installation, release, and source provenance itself. A contribution that requests a document permission MUST declare at least one document source.
...
- **FR-021a**: Every App Release MUST declare one versioned configuration schema owned by the App domain. The gateway MUST deliver the installation's validated non-secret configuration to the App inside every invocation; no provider- or environment-specific side channel may carry it. The schema MAY use the existing bounded field vocabulary for text, number, boolean, selection, URL, and connection-slot references; secret or OAuth values MUST be represented only as write-only Connection fields. The dashboard MUST render the generic schema without App-specific branches. The App runtime MUST receive only the effective non-secret configuration (stored values with manifest defaults resolved, then validated) inside each invocation; the App's only handle to a connection is the destination ID it names in an egress request, and the egress broker applies the bound credentials per the destination's declared credential-application mode.
...
- **FR-045**: An App Release MUST declare external destination patterns, protocols, ports, purpose, data classes, and, where a destination is authenticated, the connection slot and the credential-application mode (a closed catalog such as HTTP Basic, bearer token, or named header) the egress broker applies. A destination MAY mark its credentials optional so anonymous requests proceed when the slot is unbound. Installation and updates MUST display them for approval. Connection slots become required for an installation only while a contribution that declares them as required is active.
...
- **FR-076**: Release A MUST migrate every enabled WordPress connector configuration into one App Installation with an active WordPress App release, non-secret configuration holding the site URL, post types, and polling interval, a `site_credentials` Connection holding the WordPress username and application password when present, a `webhook_secret` Connection holding the shared secret, and the sync cursor as a Managed App Storage record. A push-only configuration (polling disabled) MUST migrate with no `site_credentials` Connection. Documents already ingested keep their external document IDs and source provenance so the App's next sync updates rather than duplicates them.
