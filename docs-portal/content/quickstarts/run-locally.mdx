...
Use the local backend to check registration availability, register the first user if needed, log in, issue a personal token, and call document or chat routes directly.
...
Issue a personal token with the session cookie. Add `expiresAt` only when you want the token to stop working at a known time:
...
curl -sS -b cookies.txt -X POST \
  ...
  -d '{"label":"Local development","roleCeiling":"member"}' \
  http://localhost:8080/api/v1/account/workspaces/<workspace-id>/api-access/personal-tokens
```

Store the response's `secret` value outside the browser. That personal token is what your scripts and SDK clients authenticate with once you leave the dashboard behind.
