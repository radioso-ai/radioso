...
Keep `app_base_url_override` set to the public frontend origin after the first service rollout, for example `https://app.example.com`. If you use the included Terraform workflow, set the GitHub environment variable `APP_BASE_URL` to that origin. If you deploy another way, set the backend runtime `APP_BASE_URL` to the same origin. Terraform also forwards this value as `RADIOSO_WIDGET_ORIGIN` so the website embed snippet can build hosted links.
