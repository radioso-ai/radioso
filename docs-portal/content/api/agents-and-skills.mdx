...
{
  "bundleVersion": 1,
  "agent": { "schemaVersion": 3, "name": "Support", "customInstruction": "..." },
  "routines": [
    {
      "name": "refund-flow",
      "version": 3,
...
- `skill_config_not_portable`: the source agent set skill settings whose values stay in their own workspace — a delivery webhook URL, a recipient list, a document-source scope. The `detail` names the exact settings; re-enter them on the imported skill.
...
Both `bundleVersion` and the bundle's `agent.schemaVersion` are checked against what this deployment reads. A bundle exported from a deployment on a newer version of either is rejected with `400` rather than partially applied.
