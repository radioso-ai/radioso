/**
 * Resolves the operator-facing dashboard link for a conversation. A port rather than a helper
 * because the dashboard's URL shape (workspace route key, section, query) is host routing
 * knowledge — a notification knows only that a link may exist. Returns `null` when no link can
 * be produced, for example when the workspace is gone.
 */
 ...
 * that never arrives is worse than one without a link, so this degrades to `null` instead of
 * throwing.
 */
