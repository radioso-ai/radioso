...
Radioso's MCP server runs as a standalone HTTP process. Configure `RADIOSO_BASE_URL`, `RADIOSO_MCP_SIGNING_SECRET`, and the standalone bind settings on that process, then send the original per-agent converse grant to its `/mcp` endpoint. The backend's merged setting reports `mode: "unsupported"` with `reason: "merged_auth_unavailable"` and does not mount `/mcp`; see [MCP server](/guides/mcp-server) for the client flow.

When a backend deployment has `RADIOSO_MCP_ENABLED=true`, `RADIOSO_MCP_STANDALONE=false`, and `RADIOSO_MCP_REDIS_URL` configured, startup runs a purge-only maintenance lifecycle against that Redis namespace. Backend health remains unready while the purge retries and reports ready after it succeeds. This cleanup does not expose an MCP transport.
