...
If metadata extraction is enabled for the run, the worker performs one extraction model call before chunk publication. The call understands the document type and may add structured tags such as `dateFrom` and `dateTo` to the relevant chunks.
</Steps>
