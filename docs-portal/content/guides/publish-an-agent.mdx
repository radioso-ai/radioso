...
|---|---|---|
| New conversations per calling source, across every agent | 20 an hour | `MCP_WALK_IN_SOURCE_RATE_LIMIT_MAX_ATTEMPTS` |
| New conversations per calling source, on one agent | 60 an hour | **Channels → MCP**, or `MCP_WALK_IN_AGENT_RATE_LIMIT_MAX_ATTEMPTS` |
| New conversations on one agent, from everyone | 10× the number above | `MCP_WALK_IN_AGENT_BACKSTOP_MULTIPLIER` |

The middle one is the number to tune: it is how many conversations one caller may open on your agent in an hour, and it is what stops a single looping caller from consuming the workspace's conversation allowance. Set it to the traffic you expect from one client.
