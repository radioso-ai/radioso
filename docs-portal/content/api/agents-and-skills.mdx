...
This page configures three linked concepts: [agents](/concepts), the [directives](/concepts) that steer them turn by turn, and the named [skills](/concepts) an agent can use from routines or supported invocation modes. The read-only product skills catalog still describes stable platform contracts. The host that issued your workspace token is the host these routes answer on — see [Workspaces and tokens](/api/workspaces-and-tokens).
...
skill on the same agent. Deleting that skill removes the enablement, so a
resolver-backed variable cannot remain wired to a skill that no longer exists.
