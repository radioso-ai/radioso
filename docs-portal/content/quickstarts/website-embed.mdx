...
The important idea is that Radioso does not trust any site that knows the embed token. By default, the launch also has to come from an approved origin.
...
After a successful launch, the channels settings screen shows when the website embed credential was last used. If an install code is exposed, revoke it to stop new launches immediately. Revocation does not create a replacement code; rotate the credential when you are ready to install a new one.
