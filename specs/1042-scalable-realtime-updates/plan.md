...
- **Self-hosted exact proxy**: Allows GET only, forwards a header allowlist plus abort signal, strips caller authorization/forwarding/hop-by-hop headers, streams without buffering, and allowlists SSE response headers. It appends `/api/v1/events` to a validated configured path prefix without changing the origin. Missing or invalid configuration receives only a sanitized no-store `503` with `Retry-After: 60`; transient fetch and unexpected-upstream failures receive `Retry-After: 1`. It does not modify the ordinary catch-all API proxy or claim to recover a trusted client IP from untrusted headers.
- **Event client**: Knows fetch streaming, frame parsing, terminal outcomes, stable-connection backoff, one auth refresh, and `Retry-After` as a minimum over its local jittered exponential delay for `429`/`503` only. It does not fetch dashboard resources.
- **Provider enablement boundary**: A server-derived, browser-safe `realtimeEnabled` boolean—not an internal URL—gates provider interest and connection creation. A disabled value creates no event fetch; a runtime transition to disabled closes the connection exactly once and leaves the existing poll floor authoritative.
