...
import { releaseAValidationPolicy, validateManifest } from "../src/index.js";
...
describe("reference WordPress manifest", () => {
  ...
  it("binds its only destination to the configured site URL over https", () => {
    if (!result.ok) throw new Error("fixture must validate");
    ...
    expect(destination.protocols).toEqual(["https"]);
  });
  ...
  it("reads its polling interval from configuration with a declared floor", () => {
    ...
    expect(poll.schedule).toEqual({ kind: "interval_from_configuration", field: "poll_interval_sec", minSeconds: 60 });
