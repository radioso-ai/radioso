...
AI document enrichment is an ingestion setting and is disabled by default. Operators can enable it at workspace level, override it per document source, or force it on or off for a single document, source, or workspace reprocess request. Enrichment uses one model call per processed document to classify document shape and extract supported temporal facts, which can improve event-date retrieval when the corpus contains dated events.
