...
A binding carries `channelId`, `answeringAgentId`, `escalationChannelId`, `gapEscalationEnabled`, and `respondMode`. `channelId` is the Slack channel ID — the `C…` (public) or `G…` (private) value shown at the bottom of the channel's details in Slack — not the `#name`. Slack delivers events by ID, so a binding saved under `#support` matches nothing. Omit `respondMode` on `PUT` to keep the stored value.
...
- A channel binding has no effect — mentions there use the default agent and `Every message` never fires: the binding was saved with a `#name` instead of the channel ID. Copy the `C…`/`G…` ID from the channel's details in Slack and bind that.
