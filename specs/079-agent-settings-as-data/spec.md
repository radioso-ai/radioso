...
- **G1 — RESOLVED (2026-06-05)**: authored directives are a **general behavior concept, not tied to retrieval**. They apply to **all routes by default** (empty `routes` = all routes — consistent with the existing route policy treating absent routes as all-routes). v1 does **not** surface route selection in the authoring UI/API; a directive simply applies to the agent. The `routes` column stays for future optional narrowing but is not a user-facing concept in v1. `routes` stays **backend-local** (mapped into the steering route policy at merge time; NOT added to the shared engine `Directive` contract). This supersedes the built-ins' retrieval-only scoping, which is a built-in implementation detail (org-voice/links are about presenting retrieved content), not a model for authored rules.
...
- **G6 — RESOLVED (2026-06-05)**: authored directives default to internal priority **50** at steering-map time. The operator-facing `priority` and `criticality` knobs were **removed from the authoring surface** — precedence is a system concern (matcher → coherence/disambiguation → relationship resolver), not an operator dial. The columns stay (built-ins + resolver defaults); they are no longer authored fields. The deeper direction (elevate the coherence advisory into a disambiguation flow; express precedence as directive relationships) is tracked in issue #630.
...
- **G8 — Storage model RESOLVED + import is in scope (2026-06-05)**: the "is the agent declarative if directives live in a table?" challenge is resolved. Storage = **relational-of-record + declarative projection (option A)**, not document-of-record. The child table is acceptable **on the explicit condition that export AND import both work** over it. Import is therefore a first-class goal (not just export-readiness): it reconciles the `AgentConfig` document's embedded `authoredDirectives` array into `agent_directives` rows within a transaction (insert/update/delete to match) — directives are the easy part. The **open** sub-decision is the **reference/secret re-binding policy**: export "portability mode" (redact workspace-bound refs/secrets — current serializer behavior) vs "clone mode" (preserve same-workspace `sourceScope` IDs, regenerate tokens). This also subsumes review findings M1 (`contactRequestDelivery` emitted raw) and M2 (open-ended `extensions` not registry-redacted) — both are "what does the portable document carry" decisions to settle when the export/import slice is designed.
