...
packages/
  ...
  conversation-nlp/        @radioso/conversation-nlp         (NEW)     ConversationModelGateway adapters (OpenAI…)
  conversation-tools/      @radioso/conversation-tools       (NEW)     generic tool/skill protocol (MCP/OpenAPI/local)
  conversation-kit/        @radioso/conversation-kit         (NEW)     thin HTTP server + SDK facade + CLI
```
...
contract  ◄── nlp             (the ONLY package allowed to import a provider SDK)
contract  ◄── tools
contract, engine, defaults, nlp, tools  ◄── kit   (assembles; owns no domain logic)
...
| nlp | how to call one provider behind `ConversationModelGateway` | the loop; storage; product behavior |
| tools | how to expose external functions as skills (MCP/OpenAPI/local) | which skills a product registers |
| kit | how to wire defaults+nlp+tools into a server/SDK | product rules; retrieval; workspace auth; billing |
...
- `conversation-nlp`: OpenAI `ConversationModelGateway` (then Anthropic/others); embeddings later.
...
- `conversation-kit`: HTTP server + SDK facade + CLI + in-memory wiring.
...
### Phase B — `conversation-nlp` (a provider with batteries) · spec 071
**Goal:** Ship an OpenAI `ConversationModelGateway` adapter (the port that is
vestigial today). This is the only package allowed to import a provider SDK.
