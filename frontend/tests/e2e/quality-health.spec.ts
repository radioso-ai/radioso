...
/**
 * The queue table. Returns the signal-filtered row only when the request carries
 * `signal=negative_feedback`, so a chip click is observable in the rendered table
 * rather than only in the request log.
 */
const installQualityTurnsMock = async (page: Page, requestedUrls: string[]) => {
  await page.route("**/backend/api/v1/quality/turns**", async (route) => {
    ...
    const items = signal === "negative_feedback"
