...
describe('radioso embed launcher', () => {
  ...
  it('opens embedded chat links from the host page after validating the href', async () => {
    const launcherSource = await readFile(join(process.cwd(), 'lib/radioso-embed-launcher.js'), 'utf8')
    const script = new FakeElement('script')
    script.src = 'https://app.example.com/radioso-embed.js'
    script.dataset.radiosoToken = 'embed-token'
    ...
    const messageHandlers: Array<(event: {
      source: object
      origin: string
    ...
    const window = {
      ...
      innerWidth: 1024,
      innerHeight: 768,
      addEventListener: vi.fn((event: string, handler: (event: {
        source: object
        origin: string
        data: Record<string, unknown>
      }) => void) => {
    ...
    for (const handler of messageHandlers) {
      handler({
        ...
        data: {
          type: 'radioso:embed:open-link',
          href: 'https://docs.example.com/page',
        },
      })
    }

    expect(window.open).toHaveBeenCalledWith('https://docs.example.com/page', '_blank', 'noopener,noreferrer')
