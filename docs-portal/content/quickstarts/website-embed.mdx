...
This walks you through putting the hosted Radioso website widget on a public site and proving it's locked to the site you meant. The idea worth holding onto: Radioso doesn't trust a site just because it has the embed token. By default, the launch also has to come from an origin you've approved.
...
You're not only trying to make the launcher appear. You're proving that:
...
In production, the Enterprise launcher is a normal script include with the embed token. Branding, copy, locale text, theme colors, and expert overrides are stored in Radioso settings — the agent's **Identity & appearance** screen previews them live as you edit.

![The Identity & appearance settings with assistant name, brand color, and surface style beside a live preview of the chat widget answering a question.](/screenshots/dashboard-agent-behavior.png)
...
You can generate the exact snippet from workspace settings. The detail that trips people up: the script origin and the allowed website origin are different concerns. The script can be hosted in one place while only approved customer-facing origins are allowed to bootstrap sessions.
...
The embed flow depends on the workspace public-chat capability. If website embed is enabled but the workspace has no public chat token, launch still fails. You'll find both under the agent's Channels section, alongside the Website widget, API, MCP, Slack, and WhatsApp channels.

![The agent Channels sidebar listing Public chat link, Website widget, API, MCP, Slack, and WhatsApp, with the Public chat link toggle and its last-used status shown](/screenshots/dashboard-agent-channels.png)
...
After a successful launch, the channels settings screen shows when the website embed credential was last used. If an install code is exposed, rotate the credential — the old install code stops launching new sessions the moment the token changes, and the settings screen shows the new code to deploy.
