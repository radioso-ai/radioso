...
- **Anti-Goals**: Do not weaken answer-support validation. Do not add non-grounded generic brainstorming suggestions. Do not hide suggestion behavior inside one large prompt with no explicit post-processing rules. Do not make frontend components infer broader intent solely from the displayed answer text.
