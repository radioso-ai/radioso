...
Issue a personal token with the session cookie. Pick an `expiresAt` value no more than 90 days away:
...
curl -sS -b cookies.txt -X POST \
  ...
  -d '{"label":"Local development","roleCeiling":"member","expiresAt":"<RFC3339-UTC>"}' \
