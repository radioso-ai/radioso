...
Run **Validate** before you publish. It reports problems in author terms — a branch target that no longer exists, a step that cannot reach an end, a required skill input that nothing fills, a comparison on a slot that does not exist.

Use **Save draft** to keep work in progress. Use **Publish** to create an immutable version the chat runtime runs. A routine has four statuses: `draft`, `published`, `superseded` (an older published version), and `archived` (retired). To change a published routine, choose **Edit revision** to open its draft, then publish again — the new version takes the same id in place and the previous one is marked superseded.
...
- **Test draft** runs the last *saved* draft, so save your edits first.
...
- **Test draft** shows stale behavior: it runs the last saved draft. Save, then test again.
