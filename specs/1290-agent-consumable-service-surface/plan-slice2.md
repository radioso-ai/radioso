...
## Implementation notes (configuration, as built)

- **One new deployment setting, `PUBLIC_MCP_CONVERSE_URL`.** The branch first carried eight: five rate-limit knobs and a documentation URL beside it. Those were speculative configuration for a surface with no production traffic — nobody has yet needed a number moved, and an operator cannot tell which value is right without traffic to look at. They are named constants beside the code that spends them, each carrying why that number, and they move when something asks them to. The per-agent `walkInConversationsPerHour` column stays: that is product configuration an operator reasons about, and it is the tuning lever if one is needed. The documentation URL is resolved from the shipped product-docs corpus, the same place every other docs link is resolved from, so it follows the docs rather than a deployment.
...
- **The wait is a loop in the handler around a single-shot waiter.** Each `wait` subscribes, races one jittered tick (2 000 ms ±20 %) and the remaining deadline, and resolves; the handler re-reads and either answers or waits again. A bus event published in the gap between a read and the next subscribe is therefore missed — and costs at most one tick of latency, the same bound the multi-instance case already accepts. Nothing is lost, because every wake is only a reason to re-query.
- **An absent cursor returns the most recent page, not the first one.** `messageRepository.listSinceByConversationId` with no `sinceCreatedAt` reads the newest `limit` rows; FR-040's original "from the start of this conversation" was amended to match, since `chatHistoryService` is not modified here.
- **A read reports ownership as a state alone, not the turn envelope's ack.** `ChatOwnershipAck.suppressed` answers "did the agent generate anything on this turn", and a read runs no turn; reusing the ack forced the reader to invent a value for a question nobody asked. `ConversationUpdatePage.ownership` is `ConversationOwnershipState` (`state` only), and the turn envelope keeps `suppressed` where it belongs.
- **A session with no conversation answers `200` with an empty page.** The conversation is created by the session's first turn, so a caller that reads before it asks has nothing to wait for; refusing with a 404 would make the normal first call look like a failure.
- **The page size is not on the wire.** FR-040 names `cursor` and `waitMs` only; the limit is a constant (50) so the read budget bounds work per call rather than per requested row count.
- **`agent-converse.mdx` split again, into `publish-an-agent.mdx`.** Adding resumption left it seven characters under `PAGE_INLINE_CHAR_BUDGET`, which is the same landmine slice 0 cleared. The public id, the discovery documents, and walk-in access moved to the new page (10 834 parsed chars); what a connected client does stayed (8 411). `tests/productDocsTools.test.ts` now asserts all three procedure pages come back whole.
- **The MCP adapter adds the requested wait to its transport timeout.** `RADIOSO_MCP_REQUEST_TIMEOUT_MS` defaults to 30 000, so a 25 000 ms poll would otherwise sit one network hiccup away from a client-side 504, and any operator who lowered that value would have broken long polling outright.
- **One new env var, `MCP_CONVERSE_MESSAGES_RATE_LIMIT_MAX_ATTEMPTS` (default 60), reusing `MCP_CONVERSE_SESSION_RATE_LIMIT_WINDOW_MS`.** The default is usable, so no Terraform change is required for this slice.
