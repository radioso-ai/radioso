...
description: Reach first success with plain HTTP by registering, issuing an API credential, uploading content, and asking an agent a grounded question.
...
Use the session-authenticated API-access route to create a role-bounded personal token. Its secret is shown once; inventory responses cannot recover it later. For unattended workloads, create a service-account credential instead from the same dashboard API-access area.

### Upload content and ask a grounded agent question

Use the personal token, or the service-account credential, for eligible document and chat routes.
</Steps>
...
The dashboard provides the same flow under **Settings → API access**. Copy the secret into your client's secret manager before acknowledging it; later inventory responses cannot recover it.

![Workspace Settings with separate personal-token and service-account sections in API access](/screenshots/dashboard-settings.png)
