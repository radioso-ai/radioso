...
Operators can review assistant-answer quality with `GET /api/v1/quality/turns`. A `signal` query parameter narrows the list to one class of issue: `negative_feedback`, `grounding_gaps`, `slow_responses`, or `skill_failures`. The other filters still apply on top of it. Admins and owners can update an answer's triage state with `PUT /api/v1/quality/turns/<assistant-message-id>/triage`, using one of `open`, `acknowledged`, `resolved`, or `dismissed`.
