...
- **Reviewed execution** (`operator:write`) — `execute_reviewed_proposal`, `reviewed_proposal_outcome`, and `cancel_reviewed_proposal`. First read the prepared result and show its digest, target, expected version, expiry, and draft/live effect in the conversation. Call execution only after the person confirms that exact result. The outcome reader reconciles a lost response; cancellation is available only while the same bound reviewed operation is pending and has not started execution. Execution and cancellation are keyed by their arguments, so a retry from any client reconciles with the first call rather than acting twice.
