...
Open **Settings → Service accounts**. An administrator or owner creates the stable identity and its first credential together. The form asks for the account name, role, and credential expiry; the server names the first credential `Primary`, so there is no second name to invent during setup.
