...
describe("test execution HTTP streaming", () => {
  it("reads private test history through the existing operator permission boundary", async () => {
    ...
    expect(list.body).toMatchObject({ executions: [expect.objectContaining({ id: executionId, skillEffects: "suppressed", sides: [expect.objectContaining({ revision, state: "ready" })] })] });
    ...
    expect(retained.body).toMatchObject({ id: executionId, skillEffects: "suppressed", sides: [expect.objectContaining({ conversationId: execution.sides[0].conversationId })] });

    const started = await request(app)
      .post(`/api/v1/agents/${agentId}/test-executions`)
      .set(adminSessionHeaders(session))
      .send({ mode: "single", revisionIds: ["60000000-0000-4000-8000-000000000001"], testValues: [], idempotencyKey: "idem-1", skillEffects: "allowed" });
