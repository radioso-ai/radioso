...
Radioso's MCP server runs as a standalone HTTP process. Configure `RADIOSO_BASE_URL`, `RADIOSO_MCP_SIGNING_SECRET`, and the standalone bind settings on that process, then send the original MCP-audience agent credential to its `/mcp` endpoint. See [MCP server](/guides/mcp-server) for the client flow.
