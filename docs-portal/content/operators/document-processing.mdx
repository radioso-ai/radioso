...
Local development is optimized for a simple single-machine workflow. The bootstrap path writes `backend/.env`, starts Docker Compose, and defaults document storage to the local filesystem.
