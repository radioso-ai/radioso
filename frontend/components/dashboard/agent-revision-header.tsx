...
export function AgentRevisionHeader({
  ...
  return <>
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {!canSaveDraft && state?.canPublish ? <Button type="button" size="sm" onClick={() => void review()} disabled={reviewing || !hasPublishableChanges(state)} title={hasPublishableChanges(state) ? 'Review saved draft before publishing' : 'No draft changes to publish'}>{reviewing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Review &amp; publish</Button> : null}
    </div>
