...
Everything grounded starts here. Whatever you upload is the evidence your agent answers from, so if the wrong content goes in, or processing never finishes, no later stage of retrieval can rescue the answer. Getting upload right is the highest-leverage thing you do for answer quality.
...
The Documents list is where this becomes visible. A document reads Ready once its latest revision has finished processing, and that's your signal that it's a retrieval candidate.
