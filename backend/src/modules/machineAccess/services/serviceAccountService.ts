...
import { MACHINE_ACCESS_LIFETIMES, MACHINE_ACCESS_LIMITS, assertAssignableRole, deriveCredentialStatus, normalizeCredentialLabel, normalizeCredentialExpiry, type MachineAccessRole, type ServiceAccountStatus } from "../domain.js";
...
export class ServiceAccountService {
  ...
  async createWithCredential(input: { accountId: string; workspaceId: string; actorUserId: string; displayName: string; role: MachineAccessRole; expiresAt?: Date | null }) {
    ...
    const issued = await this.input.repository.createServiceAccountWithinLimit({
      ...
      expiresAt: normalizeCredentialExpiry(input.expiresAt, this.now(), MACHINE_ACCESS_LIFETIMES.serviceDays),
