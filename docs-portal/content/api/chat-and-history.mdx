...
Use `GET /api/v1/history/chat/{conversationId}/tail` to poll for messages created after a cursor. The cursor is the newest message the client has already seen. If `cursor` is omitted, Radioso returns no messages and a baseline cursor for the current newest message. Use the full conversation route for the initial transcript, then use the tail route for near real-time updates. Dashboard tail responses can include `ownership` when the conversation is currently human-owned.
