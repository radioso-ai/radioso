...
In production, the Enterprise launcher is a normal script include with the embed token. Branding, visitor-facing wording, theme colors, and expert overrides are stored in Radioso settings — the **Web chat** page under the agent's Channels section previews them live as you edit. **Look** holds the logo, brand color, and surface style; **Wording** holds the eleven visitor-facing strings, with **Add a translation** for a per-language pack the widget serves to visitors in that language; **Footer** holds the privacy policy URL.
...
Every conversation the widget starts records what the visit looked like at that moment: the request's IP address, country, region, and city when a load balancer or CDN supplied them, the browser's `User-Agent` and `Accept-Language` header, the page the widget opened on, and the page that referred the visitor there. None of this reaches the model directly. The page, referrer, language, and region make up the `visitor_request` context variable a directive can read — see [Context variables](/operators/context-variables#visitor-request) for the six fields and how the entry page is gated. The IP address and raw browser string stay operator-only, visible in the Inbox conversation drawer's Visitor panel.

The launcher also keeps a durable visitor key in the host page's `localStorage`, scoped to the embed token so two different embeds on the same site don't collide. The key's only job is grouping every conversation that browser starts under one visitor row for operators. It does not resume a session and it does not let the widget read a previous conversation's history — each tab still opens its own chat session, and a returning visitor still starts a fresh conversation. Where `localStorage` is unavailable, such as private browsing or a storage-blocking extension, the widget falls back to per-tab behavior silently and groups nothing across visits.
...
Use the workspace embed snippet or script URL exposed by settings. The frontend serves the launcher script at `/radioso-embed.js`; it boots the widget with the public launch token and then loads server-side widget settings. Public launch-token rotation is managed from the signed-in dashboard and is separate from personal and service API credentials.
...
1. In the signed-in dashboard, open the agent's **Channels** section, enable the **Website widget**, and copy its public embed token.
...
- To see exactly what a visit records and where operators can read it, see [Visitor data](/operators/visitor-data).
