...
Because publishing the agent creates an immutable revision, you can change the onboarding flow without disturbing anyone mid-way through it. Conversations already running keep the revision they started on.
...
Radioso's [MCP server](/guides/mcp-server) exposes `ask_agent`, bound to one agent through an MCP-audience credential created from the signed-in dashboard. A tool like Cursor can hold a conversation with that agent's persona, directives, routines, and retrieval behavior without receiving a workspace role.
...
- [MCP server](/guides/mcp-server) — connect a client to one agent's conversation surface
