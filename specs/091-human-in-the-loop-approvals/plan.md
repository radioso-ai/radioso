...
**Storage**: PostgreSQL 16 (with `pgvector`, not exercised by this feature). New table `pending_decisions` (migration 104); altered `messages` (+`source`, migration 102). `routine_states` needs **no migration** — `status` is unconstrained `TEXT` so `'suspended'` is insertable as-is and `expires_at` is already nullable (no `version` column). Next migration numbers from **102** (latest is `101_agent_skills_generic_targets.sql`).
...
│   │   │  (no migration for routine_states — status is unconstrained TEXT; suspend save sets expires_at NULL)
│   │   └── 104_pending_decisions.sql                # NEW table (sibling of routine_action_requests)
│   ├── db/repositories/
│   │   ├── routineStateRepository.ts                # loadActive unchanged (status='active'); + loadSuspended({sessionId}); save sets expires_at NULL when suspended
...
- **Persistence/Integration Layer**: `pendingDecisionRepository.ts` (new, CAS resolve, one-open-per-gate unique index); `routineStateRepository.ts` (`loadSuspended({sessionId})`, `save` pausing `expires_at` when suspended); `postgresAssistantTurnPersistence.ts` (extend the single `withTransaction` to also write the pending-decision row); `actions/approvalRequestActionHandler.ts` (notification via the existing contact-delivery resolver + worker dispatcher).
...
4. **Slice A4 — durable suspend + stores (US1 / FR-002,003,004,016)**: migration `104` (`pending_decisions`) — **no routine_states migration**; `pendingDecisionRepository`; `routineStateRepository.loadSuspended` + `save` pausing `expires_at` when suspended + `loadActive` still `status='active'`; activation skips when a suspended row exists; extend `deferredRoutineStore` + `postgresAssistantTurnPersistence.completeAssistantTurn` to park-and-persist the pending-decision row atomically. Tests: integration — suspend persists atomically; `loadActive` excludes suspended; abandon sweep skips; rollback leaves no orphan.
