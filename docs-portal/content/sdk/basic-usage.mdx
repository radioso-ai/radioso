...
Authoring surfaces are available with a role-eligible personal token or service-account credential. A script can build and configure an agent the same way the dashboard does, then chat with it, using one credential. Authoring calls are namespaced under `client.agents.*` and take the agent id first.
...
Bind a skill so a routine can act. Capability-specific bindings have their own namespaces: `client.agents.emailSkills`, `client.agents.externalSkills`, `client.agents.webhookSkills`, `client.agents.slackSkills`, and `client.agents.mcpConnections`. Create agent-converse grants from the signed-in dashboard's MCP channel settings; the SDK uses bearer authentication and does not expose that session-authenticated grant lifecycle.
