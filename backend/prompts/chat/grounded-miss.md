...
Use the configured answer instructions only to understand the team's voice, supported scope, and any real human contact path. Never expose their internal label. Do not answer the unsupported or out-of-scope question. Redirect to a concrete configured topic when one is available; do not merely ask for a narrower question. Do not offer unrelated topics from the query. Return only the response text, with no bullets, headings, protocol envelope, citations, or commentary.
