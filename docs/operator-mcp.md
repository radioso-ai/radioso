...
- **Proposals** (`operator:propose`) — the agent, directive, routine, context-variable, skill, and ingestion-settings proposal tools (`propose_agent_setting`, `propose_directive`, `propose_directive_enablement`, `propose_directive_removal`, `propose_routine`, `propose_routine_edit`, `propose_routine_exposure`, `propose_context_variable`, `propose_skill_config`, `propose_ingestion_settings`, `propose_greeting`, `propose_document`, and `propose_document_retrieval`), plus `prepare_agent_settings`, `prepare_ingestion_settings`, `prepare_directive`, `prepare_routine_structure`, `prepare_retrieval_settings`, `prepare_agent_publication`, `prepare_document_import`, `prepare_document_removal`, and `prepare_document_reprocess`. `prepare_directive` prepares a directive `create`, `edit`, `set_enabled`, or `remove` operation. It retains supplied structured fields verbatim, runs coaching and the advisory coherence check only while preparing, and executes the stored reviewed payload without another model call. Removal is irreversible; its review lists directives that reference the target and recommends disabling when a reversible pause is appropriate. Directive changes alter the agent draft and require `prepare_agent_publication` before customers see them. The target's manage permission is required at preparation and execution. A prepared create is fenced only on its agent existing, so an unrelated agent change made after preparing it does not invalidate it. Reviewed edits, enablement changes, and removals use the directive version and become stale after another change to that directive executes.
