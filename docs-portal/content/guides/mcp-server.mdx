...
The current Operator MCP boundary exposes three capabilities: `operator:read` for workspace and agent state, `operator:probe` for bounded diagnostics such as retrieval probes, and `operator:propose` for reviewable ingestion-setting changes. Revision publication, private candidate testing, and frozen revision evals remain REST/dashboard operations; they are not exposed as MCP tools. `operator:act` is not part of the current tool catalog. A successful consent decision creates a grant tied to one user, workspace, client, and resource; the grant appears in the same API access card and can be revoked there.
