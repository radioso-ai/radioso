...
Radioso's standalone MCP server lets a client talk to one agent through `ask_agent`, and read Radioso's own documentation through `radioso_docs` and `radioso_doc_page`. The credential has no workspace role: it is bound to that agent and to the MCP audience.
