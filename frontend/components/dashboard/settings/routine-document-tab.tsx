...
function RoutineDocumentReader({ doc }: { doc: RoutineBlockDoc }) {
  ...
  return <article className="space-y-6 rounded-lg border border-border bg-background p-5" aria-label="Routine document">
    ...
    <section aria-labelledby="routine-document-steps"><h3 id="routine-document-steps" className="text-sm font-semibold text-foreground">Steps</h3><ol className="mt-3 divide-y divide-border">{doc.steps.map((step, stepIndex) => <RoutineStepRow key={step.stableStepId} step={step} stepIndex={stepIndex} slotNames={slotNames} />)}</ol></section>
