...
/**
 * Host-supplied availability facts about one skill. The resolver never derives
 * these — it does not know what a capability is, only that the host reported the
 * skill's capability as denied — so hosts stay free to source availability from
 * whatever policy, registry, or licence check they own.
 */
export interface DirectiveBindingSkillState {
  ...
  /** The host's capability policy denies a capability this skill requires. */
