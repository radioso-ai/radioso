...
- [ ] T020 [US1] Add `backend/src/app/http/routes/decisionRoutes.ts` (validated submit: open / member / decider-scope / hash / option → CAS → resume) + register in `backend/src/app/http/openapi/document.ts`; regenerate `backend/openapi.{yaml,json}`, `typescript-sdk` (`pnpm run sync`), MCP (`sync:openapi`).
