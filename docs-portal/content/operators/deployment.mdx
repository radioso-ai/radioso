...
`OPERATOR_MCP_RESOURCE_URL` must be the exact `/operator/mcp` URL with no query or fragment. `OPERATOR_MCP_ISSUER_URL` is the public Radioso application origin that serves authorization and consent. The frontend value is read at request time and should match the resource the standalone MCP process serves. Terraform derives these values from `operator_mcp_public_origin` and `operator_mcp_credential_epoch` when the feature is enabled.
...
The dashboard exposes `operator:read`, `operator:probe`, and `operator:propose` through the current limited catalog. Configure `OPERATOR_MCP_ROLLOUT_WORKSPACE_IDS` with one or more workspace UUIDs; an empty value exposes Operator MCP to no workspaces. `OPERATOR_MCP_VERIFICATION_BUDGET_PER_MINUTE` accepts 1 through 6 and defaults to 6. The API access card reports disabled, misconfigured, or unavailable states without offering a personal-token workaround.
