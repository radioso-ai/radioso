...
export const registerAgentsPaths = (
  ...
  registry.registerPath({ method: "put", path: "/api/v1/agents/{agentId}/greeting/draft", tags: ["Agents"], summary: "Save exact greeting content on the agent draft", operationId: "updateAgentGreetingDraft", security: revisionSecurity, request: { params: schemas.AgentParamsSchema, body: { required: true, content: { "application/json": { schema: schemas.AgentGreetingDraftSchema } } } }, responses: { 200: { description: "Draft greeting saved; validation reports field-level issues without blocking the save unless Exact words is enabled", content: { "application/json": { schema: schemas.AgentGreetingDraftResponseSchema } } }, 400: { description: "bad_request when Exact words is enabled and the content fails validation", content: { "application/json": { schema: schemas.ErrorResponseSchema } } } } });
