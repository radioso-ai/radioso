...
## Details
This sets the language for a proactive greeting when the incoming request carries no locale of its own. Pick the language by name in the setup UI; Radioso stores the matching locale tag.
...
For Exact words greetings, this locale is also the required language variant — you must supply greeting text in this language before publishing. Other language variants are optional.
