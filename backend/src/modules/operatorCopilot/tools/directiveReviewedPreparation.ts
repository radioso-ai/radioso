...
const coherenceReview = (verdict: { coherent: boolean; conflicts: ReadonlyArray<{ directiveName: string; reason: string }>; rationale: string } | null) => !verdict ? { status: "not_checked" as const, conflicts: [], rationale: "Coherence was not checked for this change." } : { status: verdict.rationale === "Coherence check unavailable." ? "unavailable" as const : verdict.coherent ? "coherent" as const : "conflicts" as const, conflicts: verdict.conflicts.slice(0, 10), rationale: verdict.rationale };
...
export const createDirectiveReviewedPreparationTool = (deps: DirectiveReviewedPreparationDependencies): CopilotToolDescriptor<z.input<typeof inputSchema>, z.infer<typeof outputSchema>> => ({
  ...
  createTool: (context) => ({ name: NAME, description: DESCRIPTION, inputSchema, outputSchema, invoke: async (raw) => {
    ...
    let payload: Record<string, unknown>; let before: AuthoredDirective | null; let after: AuthoredDirectiveInput | null; let coherence: Parameters<typeof coherenceReview>[0]; let referencedBy: ReadonlyArray<{ directiveId: string; name: string; relation: "excludes" | "dependsOn" }>; let name: string; let versionToken: string; let drafting: "verbatim" | "coach" = "verbatim";
    if (input.kind === "create" || input.kind === "edit") {
      ...
      drafting = input.name !== undefined && input.condition !== undefined && input.action !== undefined ? "verbatim" : "coach";
      ...
      ({ before, after, coherence, referencedBy } = preview); name = proposed.name; versionToken = drafted.versionToken;
      ...
      ({ before, after, coherence, referencedBy } = preview); name = after!.name; versionToken = await deps.directiveAuthor.readProposalFence(context.workspaceId, input.agentId, input.directiveId!);
      ...
      ({ before, after, coherence, referencedBy } = preview); name = before!.name; versionToken = await deps.directiveAuthor.readProposalFence(context.workspaceId, input.agentId, input.directiveId!);
      payload = { op: "remove", removesTarget: true, name, ...(input.rationale === undefined ? {} : { rationale: input.rationale }) };
    }
    const review = { kind: input.kind, target: { agentId: input.agentId, directiveId: input.kind === "create" ? null : input.directiveId!, name }, before: view(before), after: view(after), replaces: after?.excludes ?? [], referencedBy: [...referencedBy], coherence: coherenceReview(coherence), drafting, lifecycle: "agent_draft" as const, publicationRequired: true as const, irreversible: input.kind === "remove" };
