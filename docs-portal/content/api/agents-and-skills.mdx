...
- `source`: `pushed`, `browser`, or `resolver`
- `surfacing`: `always`, `on_reference`, or `operator_only`
- `enabled`

The dashboard currently exposes `pushed` and `browser` sources. Resolver-backed
variables are part of the API contract, but the operator UI does not configure
them yet.
