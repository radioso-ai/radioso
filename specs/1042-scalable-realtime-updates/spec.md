...
- **FR-016**: Connection admission, invalidation flushes, and reconnects MUST enforce configurable tenant, principal, and process limits plus abuse protection without creating high-cardinality metrics; the implementation plan MUST choose any account or network-origin dimensions and document trusted-proxy handling from the threat and capacity model.
...
- **FR-031**: Realtime-enabled startup MUST validate required transport configuration and expose health/readiness independently from the main API runtime.
...
- **FR-036**: Operator and self-hosted documentation MUST explain required infrastructure, capacity settings, health signals, failure behavior, polling fallback, and upgrade/rollback procedures.
- **FR-037**: The live client MUST treat authorization failures and a disabled endpoint as terminal poll-only outcomes, honor overload retry guidance, ignore malformed frames safely, refresh stale authentication at most once per connection attempt, and reset reconnect backoff only after a stable connection.
...
- **SC-003**: The validated fleet supports 10,000 tenants, 2,000 simultaneously active workspaces, and 100,000 concurrent visible live connections. Its acceptance run includes one workspace spread across many gateways and the hot-workspace profile defined in SC-004.
- **SC-004**: A committed-transition publisher harness sustains 5,000 post-commit invalidation requests per second for 15 minutes and injects a 50,000-request burst within one second. The test runs both a uniform distribution across 2,000 workspaces and kinds and a hot-workspace distribution with 50% of requests targeting one workspace; producer, broker, gateway, and frontend memory remain within configured bounds and return to a stable plateau. This criterion does not require unrelated product database mutations to execute at the harness rate.
...
- **SC-008**: During a one-hour soak, post-warmup gateway heap remains within 10% of its stable baseline, configured producer and fan-out queues never exceed their caps, and a blocked connection retains only one bounded convergence marker before it is eventually closed.
- **SC-009**: Cloud-hosted load tests prove the 100,000-connection fleet target through horizontal scaling while each instance stays within its configured platform concurrency and measured headroom.
- **SC-010**: A reconnect storm at the target fleet size remains within configured gateway connection-admission and API reconciliation budgets with no synchronized fleet-wide spike.
