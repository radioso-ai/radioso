...
const TARGET_LABEL = "Ingestion settings";
...
export const changedIngestionSettingKeys = (
  ...
  return Object.entries(payload)
    .filter(([key, value]) => key !== "rationale" && value !== current[key])
    .map(([key]) => key);
};
...
export const createIngestionSettingsCopilotProposalAdapter = (
  ...
  async preview(workspaceId, rawTargetRef, rawPayload) {
    ...
    const settings = await deps.ingestionSettings.getForWorkspace(workspaceId).catch(() => null);
    return {
