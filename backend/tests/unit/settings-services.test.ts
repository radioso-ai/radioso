...
describe("settings services", () => {
  ...
  it("surfaces public launch grant last-used timestamps and lifecycle status by current surface token", async () => {
    ...
    expect(result.channels).toMatchObject({
      ...
      websiteEmbedStatus: "revoked",
    });
  });

  it("revokes the current website embed grant and surfaces it as revoked", async () => {
    const workspace = {
      id: "workspace-1",
      name: "Workspace",
      assistantName: "Nora",
    ...
    let grant = createPublicLaunchGrant({ id: "embed-grant", lastUsedAt: new Date("2026-02-03T04:05:06.000Z") });
    const accessGrantService = {
      resolvePublicLaunchGrant: vi.fn(async (token: string) => token === "embed-token" ? grant : null),
      revokeGrant: vi.fn(async ({ grantId }: { grantId: string }) => {
        grant = { ...grant, id: grantId, revokedAt: new Date("2026-02-04T04:05:06.000Z") };
        return grant;
      }),
    };
    const agentService = createAgentService(createAgent(workspace));
    ...
    const result = await service.updateForWorkspace(
      "workspace-1",
      { channels: { revokeWebsiteEmbedToken: true, rotateWebsiteEmbedToken: true } },
      { accountId: "account-1" },
    );

    expect(accessGrantService.resolvePublicLaunchGrant).toHaveBeenCalledWith("embed-token");
    expect(accessGrantService.revokeGrant).toHaveBeenCalledWith({
      grantId: "embed-grant",
      accountId: "account-1",
      reason: "operator_revoked",
    });
    expect(result.channels.websiteEmbedStatus).toBe("revoked");
    expect(result.channels.websiteEmbedLastUsedAt).toBe("2026-02-03T04:05:06.000Z");
    expect(agentService.withRotatedTokens).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
  ...
  it("keeps an operator-revoked public launch grant revoked on a later settings save", async () => {
    const workspace = {
      id: "workspace-1",
      name: "Workspace",
      assistantName: "Nora",
    ...
    let grant = createPublicLaunchGrant({ id: "anonymous-grant", label: "anonymous-chat", tokenHash: "anonymous-hash" });
    const accessGrantService = {
      resolvePublicLaunchGrant: vi.fn(async (token: string) => token === "anonymous-token" ? grant : null),
      revokeGrant: vi.fn(async () => {
        grant = { ...grant, revokedAt: new Date("2026-02-04T04:05:06.000Z") };
        return grant;
      }),
    };
    const agentService = createAgentService(createAgent(workspace));
    ...
    await service.updateForWorkspace("workspace-1", { channels: { revokeAnonymousChatToken: true } });
    const laterResult = await service.updateForWorkspace("workspace-1", {
      assistant: { assistantName: "Nora Support" },
    });

    expect(accessGrantService.revokeGrant).toHaveBeenCalledTimes(1);
    expect(agentService.withRotatedTokens).toHaveBeenLastCalledWith(
      expect.anything(),
      expect.not.objectContaining({
        rotateAnonymousChatToken: true,
      }),
    );
    expect(laterResult.channels.anonymousChatStatus).toBe("revoked");
  });
