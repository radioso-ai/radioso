...
The `embed-config` response is the same for every visitor and every origin, so it can be served from a CDN. It carries no visitor or session data, and the origin allowlist is still enforced when a chat session is created. The launcher fetches this config once on page load; caching it keeps that request off your origin. After an operator changes embed settings, the change appears once the cached copy expires (or after a cache purge).
